...
```toml
sources = ["~/c/sources"]   # list of directories containing source repo checkouts
stems = "~/c/stems"         # directory where stem directories are created
...
- **WHEN** the config defines `sources = ["~/c/sources"]`
...
- **WHEN** the config defines `stems = "~/c/stems"`
